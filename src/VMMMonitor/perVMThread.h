 ...
 * @file perVMThread.h
 *
 * perVMThread class declaration
 *
 * @purpose collect performance metrics specific to a VM specified by the invoking thread;
 *
...
#ifndef perVMThread_H
#define perVMThread_H
...
//! [0]
class perVMThread : public QThread
{
    Q_OBJECT

public:
    perVMThread(QObject *parent = 0);
    ~perVMThread();
	...
	void collect(const char* machine, const char* metriclist);

	/* extra settings */
	void setParas(uint32_t samples=1, uint32_t period=1, bool detached=false, bool listmatches=false);

signals:
	/* will emit this signal when data are ready for updating the perVM stats table */
    void sigDataReady(const QString& vmName, const QString& cpuUsage, const QString& memUsage);
...
public:
	...
	/* get registered VM names */
	static size_t getAllVMs(QStringList& allVMs);
